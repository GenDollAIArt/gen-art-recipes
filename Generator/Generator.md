<!--
  ...
  Changelog:
    ...
    v9.2.2 - UI配置を整理し、「① 今回のシーンだけ（日本語で入力）」を「撮影方式 — 自撮り / 第三者撮影」の直下へ移動。撮影方式を決めた直後に場所・姿勢・身体支持・行動・表情・小物を記述でき、その後に目的・表現プリセットと個別設定を選ぶ流れへ変更。生成ロジック、シーン連動チェック、プリセット内容、A1顔ID、A4〜A9単一身体ID、バスト–ウエスト–ヒップ関係、月別ヘア、Image B、既存357系定義は変更なし。場所参照と撮影方式の古い説明文も現行仕様に合わせて整理。
    ...
    v9.2.0 - 「目的・表現プリセット」を新設し、伸びる／リアル／日常／モデル／ファッション／ムービーの6分類・48種類を追加。プリセットは既存UIのアングル、写す範囲、画面傾き、顔向き、視線、表情、身体の動き、構図、光・フィルム・トーン、エフェクト、追加要素、肌・接写質感だけを一括選択し、シーン本文や小物は変更しない。撮影方式は現在の自撮り／第三者設定を維持し、方式に応じた既存357系アングルを選ぶだけで、357系定義自体は完全維持。A1顔ID、A4〜A9単一身体ID、バスト–ウエスト–ヒップ関係、月別ヘア、Image B、参照モードをプリセットの変更対象外とするIDENTITY FIREWALLを実装。旧「撮影プリセット」の定数・状態・UI・処理を削除し、適用中／カスタム表示、再適用、解除、変更項目確認を追加。未使用の汎用細身人物ロック残骸も削除。
...
<div class="content">
  ...
  <div class="card"><div class="slabel">撮影方式 — 自撮り / 第三者撮影</div><div class="hint">最初に撮影方式を選びます。目的・表現プリセットは現在の撮影方式を維持したまま、対応する既存357系アングルと関連項目を選択します。</div><div class="chips" id="selfieModeChips"></div></div>
  ...
  <div class="card" id="visualPresetCard"><div class="slabel">目的・表現プリセット</div><div class="hint">「構図・伸びる・リアル・日常・モデル・ファッション・ムービー」から仕上がりを選び、既存の357系アングル・写す範囲・顔向き・視線・表情・構図・光・エフェクト・追加要素などを一括設定します。飲み物、ノートPC、バッグ、場所、行動などは追加せず、シーン本文をそのまま維持します。撮影方式、A1顔ID、A4〜A9身体ID、バスト–ウエスト–ヒップ関係、月別ヘア、Image B、357系定義は変更しません。</div>
...
const FACE_FOCUSED_RANGE_KEYS=new Set(["faceOnly","extremeFace","faceCloseup","profileCloseup"]);
...
const FRONT_UPPER_TORSO_AUTHORITY_ANGLE_KEYS=new Set(["tableLeanForwardOneElbow","counterLeanForwardOneElbow","tableLeanForwardBothElbows","counterLeanForwardBothElbows","lowFront","lowerBodyExtremeFront","lowerBodyExtremeFrontLookDown"]);
const THREE_QUARTER_UPPER_TORSO_AUTHORITY_ANGLE_KEYS=new Set(["lowerBodyExtremeThreeQuarter","lowerBodyExtremeThreeQuarterLookDown","tilted"]);
const SIDE_UPPER_TORSO_AUTHORITY_ANGLE_KEYS=new Set(["side","lowerBodyExtremeSide","lowerBodyExtremeSideLookDown"]);
const UPPER_TORSO_PROJECTION_AUTHORITIES={
  joint:{key:"joint",label:"A4〜A6共同照合",prompt:"This preset does not impose one explicit front, three-quarter, or profile body direction. Preserve the single upper-torso morphology jointly resolved from A4–A6. Treat the three views as mutually constraining projections of the same volume, not alternatives to average; do not choose a smaller compromise or let one view erase dimensions clearly present in another."},
  front:{key:"A4",label:"A4 正面投影を主照合",prompt:"For this explicitly frontal body projection, use A4 as the primary projection check for lateral extent, vertical placement, ribcage-to-upper-torso scale, and transition toward the waist. A5 and A6 remain mandatory cross-view checks for forward depth and three-dimensional continuity; do not let the frontal view flatten or reduce volume established across A4–A6."},
  threeQuarter:{key:"A5",label:"A5 45度投影を主照合",prompt:"For this explicitly three-quarter body projection, use A5 as the primary projection check for the combined forward depth, lateral extent, vertical placement, and transition toward the waist. A4 and A6 remain mandatory cross-view checks for width and profile depth; do not average the A5 projection down or replace it with a smaller generic compromise."},
  side:{key:"A6",label:"A6 横投影を主照合",prompt:"For this explicitly side-view body projection, use A6 as the primary projection check for front-to-back depth, outer side silhouette, vertical placement, and transition toward the waist. A4 and A5 remain mandatory cross-view checks for width and three-dimensional continuity; do not average the A6 profile depth down."}
};
const VISIBLE_BODY_REGION_PROFILES={
  ...
  headShoulder:{label:"頭・首・肩・胸郭上部",prompt:"Preserve the fixed head, neck, shoulders, and upper ribcage relationship, using A4–A6 for any visible upper-torso morphology."},
  upperBody:{label:"頭から上半身・ウエスト移行",prompt:"Preserve exactly the A4–A6-defined head-to-upper-torso relationship, including shoulders, ribcage, upper-torso size and shape, forward depth, lateral extent, vertical placement, and transition toward the waist — no enlargement, reduction, or averaging."},
  waistUp:{label:"頭から上部ヒップ",prompt:"Preserve exactly the A4–A6-defined shoulders, ribcage, upper-torso size and shape, forward depth, lateral extent, vertical placement, and torso-to-waist transition; continue through the fixed waist, pelvis top, and upper hips confirmed by A7–A9 — no enlargement, reduction, or averaging."},
  fullBody:{label:"頭から足先までの全身",prompt:"Preserve the exact A4–A6-defined upper-torso morphology within the A7–A9-confirmed full-body proportions, including fixed waist, pelvis, hips, legs, feet, hip-to-thigh continuity, and limb thickness and taper. A7–A9 must not resize or average the upper torso established by A4–A6."}
};
...
const VISUAL_PRESETS={"viralCloseNatural":{"label":"近距離ナチュラル盛れ","summary":"近距離／顔を大きく／カメラ目線／柔らかな微笑み／軽いスマホHDR","angleByMode":{"on":"golden","off":"golden","auto":"golden"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"balanced","framing":"tight","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"smile"}},"viralFaceOutfit":{"label":"顔もコーデも見えるSNS写真","summary":"腰上／顔とコーデを両立／背景を少し残す／自然な笑顔","angleByMode":{"on":"golden","off":"eye","auto":"golden"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"balanced","framing":"auto","photoStyle":"polished","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"smile"}},"viralLifestyle":{"label":"背景込みライフスタイル","summary":"腰上／場所の奥行き／人物と背景のバランス／自然な親近感","angleByMode":{"on":"high","off":"tilted","auto":"high"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"depth","framing":"standard","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr","foreground"],"additionalElements":[],"expression":"subtleSmile"}},"viralWindowBright":{"label":"窓際の明るいSNS写真","summary":"明るい窓光／顔を見やすく／透明感／輪郭光／背景も判別","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"upperBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"balanced","framing":"auto","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"transparent","effects":["hdr","rim"],"additionalElements":["windowBeam"],"expression":"smile"}},"viralNightCity":{"label":"夜の街で映えるセルフィー","summary":"夜景／光の玉／ネオン／輪郭光／被写体を明瞭に","angleByMode":{"on":"lowFront","off":"eye","auto":"lowFront"},"values":{"visibleRange":"upperBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"depth","framing":"tight","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinestill800t","tone":"dark","effects":["orb","neon","rim","hdr"],"additionalElements":[],"expression":"smile"}},"viralCommute":{"label":"通勤・移動中に映える写真","summary":"腰上／背景の場所感／清潔感／軽い傾き／自然なカメラ目線","angleByMode":{"on":"high","off":"tilted","auto":"high"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"depth","framing":"auto","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"subtleSmile"}},"viralOffice":{"label":"オフィスで映える人物写真","summary":"人物を大きく／仕事空間を残す／ガラス反射／明るく端正","angleByMode":{"on":"golden","off":"eye","auto":"golden"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"depth","framing":"auto","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"elegant","effects":["hdr","glass"],"additionalElements":["windowBeam"],"expression":"smile"}},"viralGazeHook":{"label":"視線で引き込む近接写真","summary":"顔〜肩／強いカメラ目線／余白少なめ／人物を最優先","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"headShoulder","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"subtle","framing":"full","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"confident"}},"realUnprocessedPhone":{"label":"無加工スマホ写真","summary":"目線位置／水平／自然肌／HDRなし／ボケなし／現地光","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"level","faceDirection":"auto","gaze":"auto","backgroundView":"balanced","framing":"standard","photoStyle":"amateur","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"subtle","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":[],"additionalElements":[],"expression":"neutral"}},"realUnpolishedDaily":{"label":"整いすぎない日常写真","summary":"少し不完全な構図／自然な表情／弱い手持ち感／演出最小限","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"auto","faceDirection":"auto","gaze":"off","backgroundView":"balanced","framing":"standard","photoStyle":"amateur","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":[],"additionalElements":[],"expression":"neutral"}},"realUncannyClose":{"label":"妙にリアルな近距離写真","summary":"顔〜肩／硬い窓光／鋭い影／ブラインド影／35mm日常フィルム","angleByMode":{"on":"high","off":"high","auto":"high"},"values":{"visibleRange":"headShoulder","cameraRoll":"level","faceDirection":"front","gaze":"camera","backgroundView":"subtle","framing":"tight","photoStyle":"amateur","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"superia400","tone":"minimal","effects":["hardsun","hardshadow"],"additionalElements":["blindShadow"],"expression":"neutral"}},"realHardWindow":{"label":"硬い窓光のリアル写真","summary":"斜めの直射光／窓枠影／自然肌／HDRなし／高コントラスト","angleByMode":{"on":"high","off":"tilted","auto":"high"},"values":{"visibleRange":"upperBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"balanced","framing":"auto","photoStyle":"natural","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"superia400","tone":"minimal","effects":["hardsun","hardshadow"],"additionalElements":["windowFrameShadow"],"expression":"neutral"}},"realLowLightPhone":{"label":"暗い室内の低照度スマホ","summary":"暗部を残す／高感度粒子／周辺落ち／過剰なシャドウ回復なし","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"auto","faceDirection":"threeQuarter","gaze":"off","backgroundView":"subtle","framing":"auto","photoStyle":"amateur","visualFocus":"person","motionEnergy":"subtle","effectStrength":"subtle","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"dark","effects":["grain","vignette"],"additionalElements":[],"expression":"sleepy"}},"real35mmSnapshot":{"label":"35mmフィルムスナップ","summary":"日常色／中程度の粒子／自然な視線外し／黒を潰しすぎない","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"balanced","framing":"auto","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"superia400","tone":"none","effects":["grain"],"additionalElements":[],"expression":"neutral"}},"realOldDigicam":{"label":"古いデジカメ風","summary":"正面フラッシュ／コンデジ質感／少し雑な構図／平成スナップ感","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"front","gaze":"camera","backgroundView":"balanced","framing":"auto","photoStyle":"amateur","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["digicam","flash"],"additionalElements":[],"expression":"subtleSmile"}},"realAccidental":{"label":"偶然撮れたような一枚","summary":"視線外し／AUTO傾き／自然な余白／エフェクトなし／未完成な瞬間","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"auto","faceDirection":"auto","gaze":"off","backgroundView":"balanced","framing":"standard","photoStyle":"amateur","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":[],"additionalElements":[],"expression":"neutral"}},"dailyMoment":{"label":"ふとした瞬間","summary":"自然な目線位置／視線外し／小さな動き／背景を適度に残す","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"auto","faceDirection":"auto","gaze":"off","backgroundView":"balanced","framing":"standard","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":[],"additionalElements":[],"expression":"neutral"}},"dailyCameraLook":{"label":"自然なカメラ目線","summary":"正面寄り／カメラ目線／口角だけの微笑み／被写体やや大きめ","angleByMode":{"on":"golden","off":"eye","auto":"golden"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"front","gaze":"camera","backgroundView":"balanced","framing":"tight","photoStyle":"natural","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":[],"additionalElements":[],"expression":"subtleSmile"}},"dailyLookAway":{"label":"視線を外した日常写真","summary":"斜め前／少し遠くを見る／落ち着いた空気／フィルム肌色","angleByMode":{"on":"tilted","off":"eye","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"far","backgroundView":"balanced","framing":"standard","photoStyle":"natural","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"portra400","tone":"none","effects":[],"additionalElements":[],"expression":"neutral"}},"dailyInBetween":{"label":"動作の途中を切り取る","summary":"斜め前／自然な躍動感／弱いモーションブラー／動作シーン向け","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"balanced","framing":"auto","photoStyle":"natural","visualFocus":"person","motionEnergy":"natural","effectStrength":"subtle","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"street","effects":["motionblur"],"additionalElements":[],"expression":"neutral"}},"dailyBreak":{"label":"休憩中の自然な一枚","summary":"上半身／落ち着いた視線／背景と人物を自然に両立","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"upperBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"balanced","framing":"standard","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"realaace","tone":"none","effects":[],"additionalElements":[],"expression":"subtleSmile"}},"dailyWindowProfile":{"label":"窓の外を見る横顔","summary":"真横／遠くを見る／窓光／透明感／静かな横顔","angleByMode":{"on":"side","off":"side","auto":"side"},"values":{"visibleRange":"upperBody","cameraRoll":"level","faceDirection":"profile","gaze":"far","backgroundView":"depth","framing":"auto","photoStyle":"natural","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"fuji400h","tone":"transparent","effects":["rim"],"additionalElements":["windowBeam"],"expression":"neutral"}},"dailyRelaxed":{"label":"座ってくつろぐ日常写真","summary":"目線位置／余白あり／柔らかな日常フィルム／リラックス","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"balanced","framing":"wide","photoStyle":"natural","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"polaroid","tone":"romantic","effects":[],"additionalElements":[],"expression":"subtleSmile"}},"dailyWalking":{"label":"歩きながらの自然な写真","summary":"歩行アングル／自然な躍動感／軽い粒子とブレ／歩くシーン向け","angleByMode":{"on":"walking","off":"walking","auto":"walking"},"values":{"visibleRange":"fullBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"natural","visualFocus":"balanced","motionEnergy":"natural","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"superia400","tone":"street","effects":["grain","motionblur"],"additionalElements":[],"expression":"subtleSmile"}},"modelBeauty":{"label":"王道ビューティーポートレート","summary":"顔〜肩／盛れ角／カメラ目線／自然な高品質肌／柔らかい焦点","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"headShoulder","cameraRoll":"level","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"subtle","framing":"full","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"moist","closeupTexture":"moist","film":"portra400","tone":"elegant","effects":["softfocus"],"additionalElements":[],"expression":"subtleSmile"}},"modelThreeQuarter":{"label":"斜め45度のモデルポーズ","summary":"斜め前／腰上／顔と胴体の立体感／静かな視線","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"subtle","framing":"auto","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"realaace","tone":"elegant","effects":["rim"],"additionalElements":[],"expression":"confident"}},"modelProfile":{"label":"静かな横顔ポートレート","summary":"真横／横顔アップ／遠方視線／輪郭光／モノクロの緊張感","angleByMode":{"on":"side","off":"side","auto":"side"},"values":{"visibleRange":"profileCloseup","cameraRoll":"level","faceDirection":"profile","gaze":"far","backgroundView":"subtle","framing":"tight","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"ilfordhp5","tone":"minimal","effects":["rim"],"additionalElements":[],"expression":"neutral"}},"modelFaceImpact":{"label":"顔を強く見せるアップ","summary":"顔アップ／正面寄り／強いカメラ目線／高精細／余白なし","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"faceCloseup","cameraRoll":"level","faceDirection":"front","gaze":"camera","backgroundView":"balanced","framing":"full","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"detailed","film":"portra400","tone":"minimal","effects":[],"additionalElements":[],"expression":"confident"}},"modelShoulderNeck":{"label":"肩と首筋を見せるポーズ","summary":"上半身／少し上から／斜め顔／視線外し／柔らかな輪郭光","angleByMode":{"on":"high","off":"high","auto":"high"},"values":{"visibleRange":"upperBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"subtle","framing":"auto","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"fuji400h","tone":"elegant","effects":["rim"],"additionalElements":[],"expression":"neutral"}},"modelStrongGaze":{"label":"強い視線のモデル写真","summary":"目線位置／正面／強いカメラ目線／明暗差／人物を最大化","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"headShoulder","cameraRoll":"level","faceDirection":"front","gaze":"camera","backgroundView":"balanced","framing":"full","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"dark","effects":["chiaroscuro"],"additionalElements":[],"expression":"confident"}},"modelSoftSmile":{"label":"柔らかな微笑みのモデル写真","summary":"盛れ角／顔〜肩／優しい微笑み／ポートレートフィルム","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"headShoulder","cameraRoll":"level","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"subtle","framing":"auto","photoStyle":"polished","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"moist","closeupTexture":"natural","film":"portra400","tone":"romantic","effects":["softfocus"],"additionalElements":[],"expression":"smile"}},"modelLightShadow":{"label":"光と影の人物ポートレート","summary":"斜め前／腰上／分割光／静かな表情／立体的な輪郭","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"subtle","framing":"auto","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"dark","effects":["splitlight"],"additionalElements":[],"expression":"blankGaze"}},"fashionWaist":{"label":"コーデ主役ウエストアップ","summary":"斜め前／腰上／素材とレイヤードを読みやすく／人物も維持","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"subtle","framing":"auto","photoStyle":"polished","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"realaace","tone":"elegant","effects":[],"additionalElements":[],"expression":"subtleSmile"}},"fashionFullLook":{"label":"全身コーデをきれいに見せる","summary":"全身／水平／自然な高さ／靴まで表示／背景を整理","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"fullBody","cameraRoll":"level","faceDirection":"front","gaze":"camera","backgroundView":"subtle","framing":"standard","photoStyle":"polished","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"realaace","tone":"minimal","effects":[],"additionalElements":[],"expression":"neutral"}},"fashionStreet":{"label":"街中ファッションスナップ","summary":"軽いローアングル／全身／街の奥行き／日常フィルム粒子","angleByMode":{"on":"lowFront","off":"lowFront","auto":"lowFront"},"values":{"visibleRange":"fullBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"natural","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"superia400","tone":"street","effects":["grain"],"additionalElements":[],"expression":"confident"}},"fashionOffice":{"label":"オフィスファッション","summary":"目線位置／腰上／服を主役／端正な色／ガラスと窓光","angleByMode":{"on":"golden","off":"eye","auto":"golden"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"depth","framing":"auto","photoStyle":"polished","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"realaace","tone":"elegant","effects":["glass"],"additionalElements":["windowBeam"],"expression":"subtleSmile"}},"fashionMode":{"label":"モード系ファッション撮影","summary":"斜め前／腰上／無機質な表情／分割光／暗く整理","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"subtle","framing":"auto","photoStyle":"produced","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"dark","effects":["splitlight"],"additionalElements":[],"expression":"blankGaze"}},"fashionMagazine":{"label":"雑誌風ファッション写真","summary":"構図を明確化／全身／服と人物を強く見せる／鮮明な色","angleByMode":{"on":"high","off":"eye","auto":"high"},"values":{"visibleRange":"fullBody","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"directed","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"ektar100","tone":"elegant","effects":["rim"],"additionalElements":[],"expression":"confident"}},"fashionCommerce":{"label":"商品ページ風コーデ写真","summary":"全身／水平／正面／均一で整理された構図／効果なし","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"fullBody","cameraRoll":"level","faceDirection":"front","gaze":"camera","backgroundView":"subtle","framing":"standard","photoStyle":"polished","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"minimal","effects":[],"additionalElements":[],"expression":"neutral"}},"fashionAccessory":{"label":"アクセサリーまで見せる近接写真","summary":"上半身／盛れ角／顔・首元・手元のディテール／輪郭光","angleByMode":{"on":"golden","off":"high","auto":"golden"},"values":{"visibleRange":"upperBody","cameraRoll":"level","faceDirection":"threeQuarter","gaze":"camera","backgroundView":"subtle","framing":"tight","photoStyle":"directed","visualFocus":"outfit","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"detailed","film":"portra400","tone":"elegant","effects":["rim"],"additionalElements":[],"expression":"subtleSmile"}},"movieMidScene":{"label":"映画の途中のような一枚","summary":"斜め前／視線外し／背景の奥行き／前景ボケ／輪郭光","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"produced","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinemascope","tone":"dark","effects":["foreground","rim"],"additionalElements":[],"expression":"neutral"}},"movieDayDrama":{"label":"静かな日中ドラマ","summary":"目線位置／腰上／自然光／視線外し／控えめな前景","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"directed","visualFocus":"balanced","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"portra400","tone":"minimal","effects":["foreground"],"additionalElements":["windowBeam"],"expression":"melancholic"}},"movieSunset":{"label":"夕暮れの映画的ポートレート","summary":"軽いローアングル／逆光／暖かなフレア／静かな余韻","angleByMode":{"on":"lowFront","off":"tilted","auto":"lowFront"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"produced","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"gold200","tone":"romantic","effects":["warmflare","rim"],"additionalElements":["diagonalSunbeam"],"expression":"melancholic"}},"movieNightCity":{"label":"夜の街の映画シーン","summary":"夜景／光の玉／ネオン／輪郭光／深い影","angleByMode":{"on":"lowFront","off":"eye","auto":"lowFront"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"produced","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinestill800t","tone":"dark","effects":["orb","neon","rim"],"additionalElements":[],"expression":"ennui"}},"movieWindowQuiet":{"label":"窓辺の静かなワンシーン","summary":"真横／遠方視線／窓光／前景の奥行き／静かな透明感","angleByMode":{"on":"side","off":"side","auto":"side"},"values":{"visibleRange":"upperBody","cameraRoll":"level","faceDirection":"profile","gaze":"far","backgroundView":"depth","framing":"wide","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"fuji400h","tone":"transparent","effects":["foreground","rim"],"additionalElements":["windowBeam"],"expression":"melancholic"}},"movieBacklight":{"label":"強い逆光の映画フレーム","summary":"斜め前／逆光と輪郭光／前景ボケ／フレア／演出強め","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"produced","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinemascope","tone":"dark","effects":["rim","foreground","warmflare"],"additionalElements":[],"expression":"neutral"}},"movieDeepNight":{"label":"深夜の孤独なワンシーン","summary":"暗い室内／伏し目／粒子／周辺落ち／一点の光","angleByMode":{"on":"eye","off":"eye","auto":"eye"},"values":{"visibleRange":"upperBody","cameraRoll":"level","faceDirection":"threeQuarter","gaze":"down","backgroundView":"subtle","framing":"auto","photoStyle":"directed","visualFocus":"person","motionEnergy":"subtle","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinestill800t","tone":"dark","effects":["grain","vignette"],"additionalElements":["doorCrackLight"],"expression":"sleepy"}},"movieMotionFrame":{"label":"動きの途中を切り取る映画フレーム","summary":"斜め前／自然な躍動感／モーションブラー／前景の奥行き","angleByMode":{"on":"tilted","off":"tilted","auto":"tilted"},"values":{"visibleRange":"waistUp","cameraRoll":"slight","faceDirection":"threeQuarter","gaze":"off","backgroundView":"depth","framing":"wide","photoStyle":"produced","visualFocus":"balanced","motionEnergy":"natural","effectStrength":"strong","skinFinish":"natural","closeupTexture":"natural","film":"cinemascope","tone":"street","effects":["motionblur","foreground"],"additionalElements":[],"expression":"neutral"}},"compositionTopDownFood":{"label":"真上から被写体とテーブル上の料理","summary":"90度真上／料理とテーブル面を広く／顔を上げる／レンズをチラ見","angleByMode":{"on":"trueTopDown","off":"trueTopDown","auto":"trueTopDown"},"values":{"visibleRange":"balanced","cameraRoll":"level","faceDirection":"up","gaze":"glance","backgroundView":"none","framing":"auto","photoStyle":"natural","visualFocus":"subjectFood","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"subtleSmile"}},"compositionOverheadFood":{"label":"頭上斜めから被写体とテーブル上の料理","summary":"頭上前方の斜め俯瞰／顔・上半身・料理を同時に／斜め上向き／レンズをチラ見","angleByMode":{"on":"overhead","off":"overhead","auto":"overhead"},"values":{"visibleRange":"waistUp","cameraRoll":"level","faceDirection":"diagonalUp","gaze":"glance","backgroundView":"none","framing":"auto","photoStyle":"natural","visualFocus":"subjectFood","motionEnergy":"subtle","effectStrength":"standard","skinFinish":"natural","closeupTexture":"natural","film":"none","tone":"none","effects":["hdr"],"additionalElements":[],"expression":"subtleSmile"}}};
const VISUAL_PRESET_CATEGORIES=[{"key":"composition","label":"🧭 構図","note":"真上または頭上斜めから、シーンに書いた被写体と料理・テーブル上を既存357系アングルで同時に見せる構図。","items":["compositionTopDownFood","compositionOverheadFood"]},{"key":"viral","label":"📈 伸びる","note":"SNSで顔・コーデ・場所が一目で伝わる、反応を狙う見せ方。","items":["viralCloseNatural","viralFaceOutfit","viralLifestyle","viralWindowBright","viralNightCity","viralCommute","viralOffice","viralGazeHook"]},{"key":"real","label":"📷 リアル","note":"HDRや演出を抑え、光学的不完全さと生活感を重視。","items":["realUnprocessedPhone","realUnpolishedDaily","realUncannyClose","realHardWindow","realLowLightPhone","real35mmSnapshot","realOldDigicam","realAccidental"]},{"key":"daily","label":"🌿 日常","note":"決めすぎない表情・視線・動作途中を自然に切り取る。","items":["dailyMoment","dailyCameraLook","dailyLookAway","dailyInBetween","dailyBreak","dailyWindowProfile","dailyRelaxed","dailyWalking"]},{"key":"model","label":"🧍 モデル","note":"顔、首、肩、姿勢、視線を意図的に見せる人物撮影。","items":["modelBeauty","modelThreeQuarter","modelProfile","modelFaceImpact","modelShoulderNeck","modelStrongGaze","modelSoftSmile","modelLightShadow"]},{"key":"fashion","label":"👗 ファッション","note":"Image Bを変えず、コーデ・素材・アクセサリーを読みやすく見せる。","items":["fashionWaist","fashionFullLook","fashionStreet","fashionOffice","fashionMode","fashionMagazine","fashionCommerce","fashionAccessory"]},{"key":"movie","label":"🎬 ムービー","note":"映像作品の途中のフレームとして、光・奥行き・余韻を構成。","items":["movieMidScene","movieDayDrama","movieSunset","movieNightCity","movieWindowQuiet","movieBacklight","movieDeepNight","movieMotionFrame"]}];
const VISUAL_PRESET_ALLOWED_KEYS=new Set(["angleMode","visibleRange","cameraRoll","faceDirection","gaze","backgroundView","framing","photoStyle","visualFocus","motionEnergy","effectStrength","skinFinish","closeupTexture","film","tone","effects","additionalElements","expression"]);
const IDENTITY_PROTECTED_STATE_KEYS=new Set(["characterMode","outfitReferenceMode","locationReferenceMode","selfieMode"]);
const PRESET_CONTROLLED_KEYS=new Set(VISUAL_PRESET_ALLOWED_KEYS);
...
function cloneStateValue(value){return Array.isArray(value)?[...value]:value}
function snapshotIdentityProtectedState(){const snapshot={};IDENTITY_PROTECTED_STATE_KEYS.forEach(key=>snapshot[key]=cloneStateValue(state[key]));snapshot.situation=document.getElementById("situation")?.value||"";return snapshot}
function restoreIdentityProtectedState(snapshot){IDENTITY_PROTECTED_STATE_KEYS.forEach(key=>{if(Object.prototype.hasOwnProperty.call(snapshot,key))state[key]=cloneStateValue(snapshot[key])});const scene=document.getElementById("situation");if(scene&&scene.value!==snapshot.situation)scene.value=snapshot.situation}
function currentPresetCaptureMode(){return state.selfieMode==="off"?"off":state.selfieMode==="on"?"on":effectiveSelfieForCurrentScene()}
function resolvedVisualPresetValues(preset){const values={};Object.entries(preset.values||{}).forEach(([key,value])=>values[key]=cloneStateValue(value));const mode=currentPresetCaptureMode();values.angleMode=preset.angleByMode?.[mode]||preset.angleByMode?.auto||values.angleMode||"auto";return values}
function visualPresetName(key){return VISUAL_PRESETS[key]?.label||key||""}
function activeVisualPresetKey(){return state.visualPreset==="custom"?state.visualPresetBase:state.visualPreset}
function applyVisualPreset(presetKey){const preset=VISUAL_PRESETS[presetKey];if(!preset)return;const protectedState=snapshotIdentityProtectedState();const values=resolvedVisualPresetValues(preset);Object.entries(values).forEach(([key,value])=>{if(VISUAL_PRESET_ALLOWED_KEYS.has(key))state[key]=cloneStateValue(value)});restoreIdentityProtectedState(protectedState);sanitizeEffectsForFilm();sanitizeEffectsForWeather();sanitizeAdditionalElementsForWeather();sanitizeAngleForSelfieMode();sanitizeBackgroundViewForVisualFocus();focusAngleGroupForSelection(state.angleMode);state.visualPreset=presetKey;state.visualPresetBase=presetKey}
function markVisualPresetCustom(key){if(PRESET_CONTROLLED_KEYS.has(key)&&state.visualPreset&&state.visualPreset!=="custom"){state.visualPresetBase=state.visualPreset;state.visualPreset="custom"}}
function clearVisualPreset(){state.visualPreset="";state.visualPresetBase="";renderAllOptionChips()}
function reapplyVisualPreset(){const key=activeVisualPresetKey();if(!key||!VISUAL_PRESETS[key])return;applyVisualPreset(key);renderAllOptionChips()}
function visualPresetChangeLines(key){const preset=VISUAL_PRESETS[key];if(!preset)return[];const values=resolvedVisualPresetValues(preset);const order=["angleMode","visibleRange","cameraRoll","faceDirection","gaze","expression","motionEnergy","visualFocus","backgroundView","framing","photoStyle","film","tone","effects","additionalElements","effectStrength","skinFinish","closeupTexture"];const names={angleMode:"アングル",visibleRange:"写す範囲",cameraRoll:"画面の傾き",faceDirection:"顔の向き",gaze:"視線",expression:"表情",motionEnergy:"身体の動き",visualFocus:"表現の主役",backgroundView:"背景の見せ方",framing:"被写体サイズ・余白",photoStyle:"写真の作り込み度",film:"フィルム",tone:"作品トーン",effects:"エフェクト",additionalElements:"追加要素",effectStrength:"エフェクト強度",skinFinish:"肌の見え方",closeupTexture:"接写質感"};return order.filter(k=>Object.prototype.hasOwnProperty.call(values,k)).map(k=>{let label="";if(k==="effects")label=values[k].length?values[k].map(effectLabel).join("、"):"なし";else if(k==="additionalElements")label=values[k].length?values[k].map(v=>ADDITIONAL_ELEMENTS.find(x=>x.key===v)?.label||v).join("、"):"なし";else{const item=getItems(k).find(x=>x.key===values[k]||x.value===values[k]);label=item?.label||values[k]||"指定なし"}return `${names[k]}：${label}`})}
...
function makeMultiChipButton(item,key){
  ...
  btn.onclick=()=>{if(disabled)return;markVisualPresetCustom(key);const idx=state[key].indexOf(item.key);if(idx<0)state[key].push(item.key);else state[key].splice(idx,1);renderAllOptionChips();};
  const helpItem=reason?{...item,help:(item.help?item.help+"\n\n":"")+"選択不可\n"+reason}:item;bindChipHelp(btn,helpItem);return btn;
}
...
function getVisualPresetSceneValidationErrors(){
  const key=activeVisualPresetKey();
  const foodPreset=key==="compositionTopDownFood"||key==="compositionOverheadFood";
  if(!foodPreset||state.visualFocus!=="subjectFood")return[];
  const expectedAngle=key==="compositionTopDownFood"?"trueTopDown":"overhead";
  if(state.angleMode!==expectedAngle)return[];
  const name=VISUAL_PRESETS[key].label;
  const errors=[];
  if(!sceneHasCue("table")&&!sceneHasCue("counter"))errors.push(`『${name}』には、シーン欄にテーブルまたはカウンターの記述が必要です。`);
  return errors;
}
function collectSceneValidationErrors(){
  ...
  return [...new Set([...checks.map(([key,itemKey])=>getSceneLinkedDisabledReason(key,itemKey)).filter(Boolean),...getVisualPresetSceneValidationErrors()])];
}
function updateSceneValidationStatus(){
  ...
  el.textContent="シーン連動チェック：場所・姿勢・行動・小物を入力すると、成立しない選択肢を自動で無効化します。";
}
...
function resolveUpperTorsoProjectionAuthority(angleKey){
  if(FRONT_UPPER_TORSO_AUTHORITY_ANGLE_KEYS.has(angleKey))return UPPER_TORSO_PROJECTION_AUTHORITIES.front;
  if(THREE_QUARTER_UPPER_TORSO_AUTHORITY_ANGLE_KEYS.has(angleKey))return UPPER_TORSO_PROJECTION_AUTHORITIES.threeQuarter;
  if(SIDE_UPPER_TORSO_AUTHORITY_ANGLE_KEYS.has(angleKey))return UPPER_TORSO_PROJECTION_AUTHORITIES.side;
  return UPPER_TORSO_PROJECTION_AUTHORITIES.joint;
}
function resolveBodyProjectionProfile(angleKey,stateRange=state.visibleRange){
  if(state.characterMode!=="on")return{key:"none",label:"参照なし",regionLabel:"参照なし",projectionLabel:"参照なし",authorityLabel:"参照なし",prompt:""};
  const region=resolveVisibleBodyRegion(stateRange);
  const projection=resolveAngleBodyProjection(angleKey);
  const authority=resolveUpperTorsoProjectionAuthority(angleKey);
  return{
    key:`${region.key}:${projection.key}:${authority.key}`,
    label:`${region.label} ＋ ${projection.label} ＋ ${authority.label}`,
...
${projection.prompt}
ANGLE-MATCHED UPPER-TORSO REFERENCE CHECK:
${authority.prompt}
This angle-matched reference is a projection validation of the already fixed canonical body; it must never create a separate body ID or recalculate the morphology.
...
上半身投影照合：生成時の方向に応じてA4／A5／A6または共同照合`;
    return;
  }
  const projection=resolveAngleBodyProjection(state.angleMode);
  const authority=resolveUpperTorsoProjectionAuthority(state.angleMode);
  el.textContent=`写す身体領域：${region.label}
アングル投影：${projection.label}
上半身投影照合：${authority.label}
...
No Images A1 through A9 are used. Derive one clearly adult subject naturally from the written scene and active photographic settings.`;return`SUBJECT ID CORE — A1–A9 — MANDATORY:

REFERENCE MAP:
A1 = sole face-identity source. A2–A3 = angle confirmation of that same face only.
A4–A6 = upper-body front / three-quarter / profile. A7–A9 = full-body front / three-quarter / profile.
A4–A9 contain body information only; do not use their faces. A1–A9 do not control outfit, hair, pose, camera, location, lighting, or scene content.

FACE ID — A1 AUTHORITY:
Resolve and freeze one recognizable adult face from A1. Use A2–A3 only to confirm that identity across angles; never average, replace, beautify, normalize, or redesign A1.
Preserve A1's defining facial proportions and eye, eyelid, eyebrow, nose, lip, cheek, jaw, chin, and forehead structure.

CANONICAL BODY ID — A4–A9 / REGION-SPECIFIC AUTHORITY:
Resolve and freeze one fixed three-dimensional body across A4–A9 before outfit, pose, camera, crop, lighting, or effects.

UPPER-TORSO SOURCE AUTHORITY — A4–A6 CROSS-VIEW CONSENSUS:
A4–A6 jointly define one upper-torso morphology of the same canonical body. Treat A4 front, A5 three-quarter, and A6 profile as mutually constraining projections of one fixed volume, not separate bodies and not alternatives to average into a smaller compromise.
A4 confirms lateral extent and the torso-to-waist relationship. A5 confirms forward projection and lateral volume together. A6 confirms front-to-back depth and the outer side silhouette. Preserve every dimension that is clearly supported across these views — no more and no less — and do not let one view erase, reduce, or normalize dimensions revealed by another.

FULL-BODY INTEGRATION — A7–A9:
A7–A9 confirm stature, overall proportions, waist, pelvis, hips, limbs, and how the same A4–A6-defined upper torso connects into the full body. They must not average down, enlarge, reduce, reshape, normalize, or replace the upper-torso morphology established by A4–A6.

Keep all regions as one connected canonical body. Do not idealize, flatten, or reconstruct it for clothing, pose, crop, camera direction, or generic human priors.`}
function outfitBlock(){if(state.outfitReferenceMode==="off")return`OUTFIT REFERENCE — OFF:
Do not use Image B. Create one coherent outfit from the written scene, weather, time, and selected style. Do not reuse a previous or cached outfit.`;return`OUTFIT REFERENCE — IMAGE B ONLY — STRICT:
...
${state.characterMode==="on"?"Fit Image B garments over the exact canonical body, using the cross-view A4–A6 consensus as the authoritative source for upper-torso morphology. The completed garment silhouette must reflect the size, shape, forward projection, lateral extent, vertical placement, and transition toward the waist jointly established by A4–A6 — no more and no less. Adjust garment cut, stretch, looseness, folds, tension, and drape around that fixed morphology. Loose or fully opaque fabric may hide fine surface contours, but it must not enlarge, reduce, average down, flatten, or replace the body-supported overall volume and front/side silhouette. A7–A9 confirm full-body continuity only and must not override the A4–A6 upper-torso morphology.":"Image B must not impose the body shown in the reference; it controls clothing only."}`}
...
Do not use Image C. The written scene is the sole source for the current place, spatial use, subject placement, action, and props.`;
  return`LOCATION CORE — IMAGE C + WRITTEN SCENE:
Image C controls only the base environment: architecture, materials, layout, vegetation, fixtures, and ambient-light character.
Reconstruct that environment from the selected camera geometry; do not paste or trace C as a flat background. Ignore people and all subject, outfit, pose, hand, camera, and expression information in C.
The written scene controls the current use of the place, exact subject placement, posture, action, physical support, and props. When scene details differ from incidental contents in C, follow the written scene while preserving C's environmental identity.
Integrate subject and place with coherent perspective, scale, ambient light, contact shadows, depth, reflections, and light spill.`;
}
function monthlyHairBlock(month,subjectReferenceOn){
  ...
  return `MONTHLY HAIR — SOLE HAIR SOURCE — MANDATORY:
${base},
${strict},
(the monthly style alone controls cut, length, color, silhouette, parting, volume, and finish:2.0),
(${subjectReferenceOn?"ignore all hair visible in A1–A9":"do not inherit hair from prior images or conversation"}:2.0),
(Image B and Image C do not control hair:2.0),
(scene may change only temporary strand motion or a removable head accessory without changing the locked hairstyle:1.98)`;
}
function finalFaceIdentityCheck(){
  if(state.characterMode!=="on")return"";
  return `FINAL FACE ID CHECK — A1:
Confirm the visible face is still the same recognizable A1 identity. A2–A3 only confirm angle; A4–A9 provide no face information.
If facial proportions or defining eyes, brows, nose, lips, cheeks, jaw, chin, or forehead were averaged, beautified, or redesigned, restore A1 before finalizing.`;
}
function finalBodyMorphologyCheck(angleKey,rangeKey){
  if(state.characterMode!=="on")return"";
  const profile=resolveBodyProjectionProfile(angleKey,rangeKey);
  return `FINAL BODY ID CHECK — A4–A9 / REGION AUTHORITY:
Confirm that the visible projection and anatomically continuous off-frame body remain the same canonical identity under crop-required body regions “${profile.regionLabel}”, angle projection “${profile.projectionLabel}”, and upper-torso projection check “${profile.authorityLabel}”. Verify the completed body-supported garment silhouette against that angle-matched A4, A5, A6, or joint cross-view check without enlargement, reduction, flattening, or averaging down. The matched view is only a projection check; A4–A6 jointly define one fixed morphology, and A7–A9 only confirm full-body proportions and continuity. If any region was normalized, altered by garment drape, or replaced by a generic body, restore the canonical body before finalizing.`;
}
...
${state.locationReferenceMode==="on"?"C=C_LOCATION_REFERENCE.jpg (final active reference).":"Location reference OFF: no C."}
Use fixed project-source files when available and do not request re-upload. Otherwise attach all active files together in A1–A9 → B → C order.`);
...
blocks.push(`WRITTEN SCENE — CURRENT PLACE / ACTION / PROPS:
${sceneRaw}
The written scene is authoritative for the current place and its use, subject placement, standing / seated / reclining / leaning state, physical support, action, expression detail, and all props or food.
Keep body contact, support, object placement, scale, and reachability physically coherent. When Image C is active, C supplies the environmental base while this scene supplies the current event and local arrangement.
The scene must not override A1 face ID, A4–A9 body ID, Image B outfit when active, monthly hair, selected 357-series angle, or selfie camera / hand roles.`);
blocks.push(`CAMERA MODE:
${cameraModeBlock(selfie,angle.key)}`);
blocks.push(buildAngleExecutionPrompt(angle,selfie));
const selectedBodyData=bodyProjectionBlock(angle.key,state.visibleRange);if(selectedBodyData)blocks.push(selectedBodyData);
