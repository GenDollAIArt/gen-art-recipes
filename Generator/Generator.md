...
function resolveMotion(situation, dayMood, timeCtx, manualTension, manualEmotion, scene, angleName, subjectSizeKey, photoStyleKey) {
  ...
  } else if (angleName === "SIDE PROFILE") {
    anglePose = "true side-view camera position, profile or near-profile face angle, camera stays around eye height, lateral face line and hair silhouette are readable, not front-facing, not overhead, " + chinPose;
  } else if (angleName === "RECLINING SIDE EYE LEVEL") {
    anglePose = "reclining side eye-level pose, camera beside her face at the same height, sideways resting posture, natural eye contact from a lying or reclining position, not overhead, not strong low-angle, " + chinPose;
  } else if (angleName === "BACK VIEW") {
    anglePose = "external camera behind the subject, back-view fashion posture, face mostly hidden, shoulders and back of outfit visible, looking away from camera, no selfie arm, no front chest emphasis, " + chinPose;
  } else if (angleName === "DIAGONAL BACK VIEW") {
    anglePose = "external camera from behind and slightly to the side, three-quarter rear pose, slight side profile hint, shoulder line and backlit outfit edge visible, no selfie arm, no front chest emphasis, " + chinPose;
  } else if (angleName === "BACK OVER SHOULDER") {
    anglePose = "external over-the-shoulder view from behind, shoulder and hair foregrounded, slight glance back if natural, rear-to-side fashion posture, no selfie arm, no front chest emphasis, " + chinPose;
  } else if (angleName === "WINDOW BACK VIEW") {
    anglePose = "external camera behind her near the window, she looks outside, back of head, hair, shoulders and opaque clothing edges outlined by room lamp or window light, no selfie arm, no front chest emphasis, " + chinPose;
  ...
  } else if (getAngleMeta(angleName).rear) {
    scenePose = "quiet external-camera fashion posture, body naturally placed in the room or location, looking away or toward the window, no selfie behavior";
  } else {
...
function buildBodySilhouetteBlock(subjectSizeMode, bustPrompt, hipPrompt, angleName, sceneText = "") {
  ...
  if (isFaceCloseupSubjectKey(subjectKey)) {
    return `BODY / CLOTHING:
(outfit is visible only minimally because this is a face close-up:1.85),
(opaque casual fashion clothing, no transparency, no see-through fabric, no body-emphasis:1.92),
(face, eyes, hair, expression, and lighting are the primary focus:1.95),
(do not emphasize chest, hips, waist, or anatomy in this close-up composition:1.95)`;
  }

  if (getAngleMeta(angleName).rear) {
    return `BODY / CLOTHING SILHOUETTE:
(realistic mature adult proportions, slim natural body line, no exaggerated body proportions:1.85),

BACK / SIDE OUTFIT SILHOUETTE:
(back, shoulders, hair, and outer clothing shape are the readable fashion elements:1.88),
(opaque garment surface and edge-lighting are visible without explicit body detail:1.9),
(no front chest emphasis, no see-through body detail, no erotic framing:1.95),
(the selected clothing silhouette should remain tasteful and fashion-oriented:1.88)`;
  }

  if (getAngleMeta(angleName).side || getAngleMeta(angleName).reclining) {
    return `BODY / CLOTHING SILHOUETTE:
...
(no forced chest or hip emphasis beyond the outfit design:1.9)`;
  }
...
function buildPrompt(t, situation, characterLock, bustPrompt, hipPrompt, skinFinishPrompt, weatherVal, filmVal, toneVal, effectsArr, effectStrengthMode, subjectSizeMode, backgroundViewMode, framingMode, photoStyleMode, angle, expression, scene, accessories, motionText) {
  const overviewParts = [];
...
function handleGenerate() {
  ...
  const prompt = buildPrompt(t, situation, characterLock, bustPrompt, hipPrompt, skinFinishPrompt, state.weather, state.film, state.tone, state.effects, effectStrengthMode, subjectSizeMode, backgroundViewMode, framingMode, photoStyleMode, angle, expression, scene, accessories, motionText);
