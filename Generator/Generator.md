...
<div class="header">
  ...
  <div><div class="header-title">Stable Character Prompt Generator</div><div class="header-sub">v9.2.0</div><div class="header-sub">参照/空間演出/拡張エフェクト</div></div>
...
<div class="content">
  ...
  <div class="card" id="visualPresetCard"><div class="slabel">目的・表現プリセット</div><div class="hint">「構図・伸びる・リアル・日常・モデル・ファッション・ムービー」から仕上がりを選び、既存のアングル・写す範囲・顔向き・視線・表情・構図・光・エフェクト・追加要素などを一括設定します。飲み物、ノートPC、バッグ、場所、行動などは追加せず、シーン本文をそのまま維持します。撮影方式、A1顔ID、A4〜A9身体ID、バスト–ウエスト–ヒップ関係、月別ヘア、Image B、357系定義は変更しません。</div>
...
function getVisualPresetSceneValidationErrors(){
  ...
  if(key!=="compositionTopDownFood"||state.angleMode!=="trueTopDown"||state.visualFocus!=="subjectFood")return[];
  const errors=[];
  if(!sceneHasCue("table")&&!sceneHasCue("counter"))errors.push("『上から被写体とテーブル上の料理』には、シーン欄にテーブルまたはカウンターの記述が必要です。");
  if(!sceneHasCue("food"))errors.push("『上から被写体とテーブル上の料理』には、シーン欄に料理・食事・食べ物などの記述が必要です。");
  return errors;
}
...
Before applying hairstyle, outfit, pose, camera, crop, face direction, gaze, expression, lighting, skin finish, film, or effects, resolve one face identity from A1 as the sole visible facial source.
Freeze that exact A1 face identity once resolved and keep the same face instance throughout the complete generation process.
...
PRESENTATION CONTROL / IDENTITY FIREWALL — MANDATORY:
Resolve and freeze the A1 face identity and the joint A4–A9 canonical body identity before any camera, crop, face-direction, gaze, expression, motion, lighting, film, tone, effect, or atmospheric control is applied.
All presentation controls are downstream appearance instructions only. They must not trigger a new interpretation, averaging, beautification, normalization, or reconstruction of the face or body.
The locked bust–waist–hip relationships, shoulder-to-pelvis relationship, torso length, limb proportions, and continuous bodyline must remain invariant beneath every selected presentation treatment.
The selected 357-series angle remains the authoritative completed camera geometry and must be applied to the already frozen identity without changing that identity.
...
function handleReset(){state={...DEFAULT_STATE,effects:[],additionalElements:[]};document.getElementById("situation").value="";document.getElementById("metaCard").classList.add("hidden");document.getElementById("outputCard").classList.add("hidden");document.getElementById("outputArea").value="";renderAllOptionChips();}
