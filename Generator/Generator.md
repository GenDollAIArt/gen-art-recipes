...
function detectPoseContext(sceneText = "") {
  ...
  const support = /壁|wall/i.test(t) ? "wall" : /机|テーブル|カウンター|desk|table|counter/i.test(t) ? "desk" : /椅子|ソファ|chair|sofa/i.test(t) ? "chair" : floorSeated ? "floor" : "none";
...
function buildPosturePrompt(sceneText, scene, poseCtx) {
  if (poseCtx.floorSeated) return "floor-seated posture, sitting on the floor with knees or folded legs naturally arranged, body supported by the floor" + (poseCtx.support === "wall" ? ", back or shoulder lightly supported by the wall" : "") + ", compact stable seated balance";
  ...
  if (poseCtx.support === "wall") return "standing near the wall with subtle support, shoulders and spine remain naturally aligned";
