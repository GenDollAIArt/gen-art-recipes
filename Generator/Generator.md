...
<div class="header">
  ...
  <div>
    ...
    <div class="header-sub">固定キャラ + 今回のシーン v4.0.5.3</div>
  </div>
...
const APP_VERSION = "v4.0.3-more-effects";
...
function flashSnapshotEffectValues() {
  // 直フラッシュ/白飛びフラッシュは、被写体に正面から強い光が当たるスナップ系。
  // 背景ボケ・玉ボケ・自然光/逆光系と同時に入れると、主光源の方向性が割れて違和感が出やすい。
  return [
    currentEffectValue("直フラッシュ"),
    currentEffectValue("白飛びフラッシュ"),
  ].filter(Boolean);
...
function softDepthLightEffectValues() {
  // 自然光/背景描写/浅い被写界深度系。フラッシュスナップ系とは原則どちらか一方にする。
  return [
...
function isIncompatibleOption(stateKey, itemValue) {
  ...
  if (stateKey === "effects") {
    ...
    // フラッシュスナップ系と背景ボケ/自然光/逆光系は主光源の方向が割れるため同時選択不可
    ...
    // 硬い影と、やわらかい影/ソフトフォーカスは影の質が反対なので同時選択不可
...
const GARMENT_BACKLIGHT_OPTIONS = [
  {label:"🚫 指定なし", value:""},
  {label:"☀️ 光を通す", value:"(backlight softly glows through the garment fabric:1.82), (delicate thin fabric catches sunlight from behind:1.78), (airy light-transmitting cloth impression:1.76), (non-explicit fashion portrait:1.9), (face and outfit atmosphere remain primary:1.9)"},
  {label:"🫧 ふんわり透け感", value:"(soft translucent fabric impression in backlight:1.82), (fabric appears light and airy without explicit body detail:1.86), (subtle silhouette impression only through layered fabric:1.78), (no visible nipples, no explicit anatomy, no erotic emphasis:1.95), (fashion mood remains primary:1.9)"},
  {label:"🌙 逆光シルエット", value:"(strong backlight reveals a tasteful soft silhouette through the garment:1.82), (silhouette readable but non-explicit and fashion-oriented:1.88), (no explicit body details, no visible nipples, no fetish styling:1.95), (artistic backlit fashion image, not erotic:1.95), (face and atmosphere remain primary:1.9)"}
];
...
// ── Long-press chip help (effects only) ─────────────────────────────────────
...
function buildPrompt(t, situation, characterLock, bustPrompt, hipPrompt, skinFinishPrompt, weatherVal, filmVal, toneVal, effectsArr, effectStrengthMode, subjectSizeMode, backgroundViewMode, framingMode, photoStyleMode, angle, expression, scene, accessories, motionText) {
  ...
  const garmentLightPrompt = state.garmentLight || "";
